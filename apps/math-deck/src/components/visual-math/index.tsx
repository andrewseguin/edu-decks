...
export function VisualMath({ problem, isFlipped = true }: VisualMathProps) {
  ...
  const clearStepTimers = () => {
    if (stepTimersRef.current.t0) clearTimeout(stepTimersRef.current.t0);
  ...
  useEffect(() => {
    ...
    // Reset state for new problem - start at 0 so incremental step 1 timers fill cleanly from empty
    setCyanVisible(0);
    setOrangeVisible(0);
    setSubtractionCount(0);
    setWhiteCount(0);
    ...
    const startDelay = replayKey === 0 ? 500 : 0;
    stepTimersRef.current.t0 = setTimeout(() => {
      if (problem.isFraction && problem.frac1 && problem.frac2) {
        const f1 = problem.frac1;
        const f2 = problem.frac2;
        const commonD = lcm(f1.d, f2.d);
