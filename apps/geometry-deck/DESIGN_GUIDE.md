...
> [!IMPORTANT]
> **Touch Points & Draggable Handles (Always White)**: All interactive drag handles, touch points, and vertex manipulation anchors must strictly remain **White** (`fill="white"`, outer grab ring `fill="rgba(255,255,255,0.15)" stroke="rgba(255,255,255,0.5)"`). Semantic colors are strictly reserved for mathematical properties (angles, lengths, heights, areas), while white serves as the universal, consistent affordance for interactive touch control.
